...
select first_name, job_id from employees where job_id='ST_CLERK'; -- 5개
...
select 
 ...
 select 
    ...
    ltrim(to_char(sum(salary), '$999,000')) "통화기록삽입"
...
/*
count() : 그룹화된 레코드의 개수를 카운트 할때 사용하는 함수
*/
...
select
    ...
    count(distinct job_id) "순수담당 업무 갯수"
 from employees;
...
select
 ...
 /*
 정보 검색시 대소문자 혹은 공백이 포함된 경우 
 모든 레코드에 대해 문자열을 확인하는 것은 불가능하므로
 일괄적인 규칙의 적용을 위해 upper()와 같은 변환함수를 사용하여
 검색하는 것이 좋다.
 */
 ...
 */
 ...
 /*
    ...
    가장 낮은 급여를 받는 사람은 아래와 같이 서브쿼리를 통해 구할 수 있다.
...
/*
group by절 : 여러개의 레코드를 하나의 그룹으로 그룹화하여
묶여진 결과를 반환하는 쿼리문
...
/*
step1 : 부서가 많은 경우 일일이 부사별로 확인할 수 없으므로
부서를 그룹화한다. 중복이 제거된 결과로 보이지만 동일한 레코드가
하나의 그룹으로 합쳐진 결과가 인출된다.
*/
select department_id from employees group by department_id;
    
/*
step2 : 각 부서별로 급여의 합계를 구할 수 있다.
4자리가 넘어가는 경우 가독성이 떨어지므로 서식을 이용해서 
세자리마다 콤마를 표시한다.
*/
...
/*
앞에서 사용했던 쿼리문을 아래와 같이 수정하면 에러가 발생합니다.
group by 절에서 사용한 컬럼은 select절에서 사용할 수 있으나,
그 외의 단일 컬럼은 select 절에서 사용할 수 없다.
그룹화된 형태에서 특정 레코드 하나만 선택하는 것은 애매하기 때문이다.
*/
select 
...
/*
시나리오] 부서아이디가 50인 사원들의 직원총합, 평균 급여,
급여총합이 얼마인지 출력하는 쿼리문을 작성하시오.
*/
select 
 ...
 */
 /*
 같은 부서에 근무하더라도 담당 업무는 다를 수 있으므로
 이 문제에서는 group by 절에 2개의 컬럼을 명시해야 합니다.
 즉 부서로 그룹화 한 후 다시 담당 업무별로 그룹화한다.
 */
 select 
...
/*
    ...
    where절에 쓰면 에러가 발생합니다. 이런 경우에 having절에
    조건을 추가해야 한다. 
    Ex) 급여가 3000원인 사원 -> 물리적으로 존재하므로 
        where절에서 사용
        평균 급여가 3000인 사원 => 논리적으로 존재하므로
        having절 사용
        즉, 그룹함수를 통해 구할 수 있는 데이터
*/    
 select 
    department_id, job_id, count(*), avg(salary)
    from employees
    group by department_id, job_id
    having count(*)>10; -- 그룹에 조건은 having절에 기술한다.
    
/*
퀴즈] 담당업무별 사원의 최저급여를 출력하시오.
단, 관리자(manager)가 없는 사원과 최저급여가 3000미만인 그룹은 제외
결과를 급여의 내림차순으로 정렬하여 출력
*/    
select
    job_id, min(salary)
    from employees
    where manager_id is not null -- 테이블에서 찾을 수 있으면 where
    group by job_id
...
/*
    ...
    min(salary)를 사용해야 합니다.
*/

-- #해당 문제는 hr 계정의 employees 테이블을 사용합니다.

/*
1. 전체 사원의 급여최고액, 최저액, 평균급여를
출력하시오. 컬럼의 별칭은 아래와 같이 하고,
평균에 대해서는 정수형태의 반올림 하시오
별칭) 급여최고액 -> MaxPay
급여최저액 -> MinPay
급여평균 -> AvgPay
round(), to_char() : 반올림 처리되어 출력함
trunc() : 소수 이하를 잘라서 출력됨, 반올림 되지 않음
*/
select
...
--    trunc(avg(Salary)) AvgPay3,
    ...
    from employees;
/*    
2. 각 담당업무 유형별로 급여최고액, 최저액,  총액 및
평균액을 출력하시오. 컬럼의 별칭은 아래와 같이하고 모든 숫자는
to_char을 이용하여 세자리마다 컴마를 찍고 정수형태로 출력하시오
별칭) 급여최고액 -> MaxPay
급여최저액 -> MinPay
급여평균 -> AvgPay
급여총액 -> Sumpay
...
출력하시오.
참고] employees 테이블의 job_id컬럼을 기준으로 한다
-물리적으로 존재하는 컬럼이 아니라면 함수 혹은
-수실 그대로 order by절에 기술하면 된다

...
Select * From tab; -- 계정마다 모두 다르다
/*
Dual 테이블
    : 하나의 행으로 결과를 출력하기 위해 제공되는 테이블로
    오라클에서 자동으로 생성되는 논리적 테이블이다.
    varchar2(1)로 정의된 dummy(가상)라는 단 하나의 컬럼으로 구성되어 있다
*/

select * From dual; -- X가 있다.
...
select trunc(12345.12345, -2) from dual;
-- 금액이라면 100단위 이하 절삭과 같은 형태로 활용할 수 있다.

/*
시나리오] 사원 테이블에서 영업사원이 1000불에 대한 커미션을 계산하여
    급여에 합한 결과를 출력하는 쿼리문을 작성하시오.
    Ex) 급여 : 1000, 보너스율 : 0.1
        -> 1000 + (1000*0.1) = 1100
*/
-- 1. 영업 사원을 먼저 찾아 인출한다. (영업사원은 job_id가 SA_XX로 되어있다.
select * From employees where job_id like 'SA_%';
--( 영업사원은 커미션을 받기 때문에 값이 저장되어 있다.)
...
select First_NAME, salary, trunc(commission_pct,1), 
    (salary + trunc(commission_pct,1))
        from employees where job_id like 'SA_%';
-- 4. 계산식이 포함된 컬럼명을 별칭을 부여한다.
select First_NAME, salary, trunc(commission_pct,1) comm_pct, 
    (salary + trunc(commission_pct,1)) as TotalSalary
        from employees where job_id like 'SA_%';
/*
연습문제 : 사원테이블에서 보너스율이 있는 사람만 인출한 후
보너스들을 소수점 1자리로 표현하시오
출력내용 : 이름, 급여, 보너스율
*/
-- 1. 커미션이 있는 사원만 인출
Select First_name, last_name, salary, commission_pct from employees
    where commission_pct is not null; -- not null 있는 사람만
-- 2. 소수점 처리하기
Select First_name, last_name, salary, trunc(commission_pct,1) from employees
...
소수점 관련 함수
    ...
    floor() : 무조건 버림처리
    round(값, 자리수) : 반올림 처리
        두번째 인자가 없는 경우 : 소수점 첫번째 자리가 5이상이면 올림, 미만이면 버림
        있는 경우 : 숫자만큼 소수점이 표현되므로 그 다음수가 5이상이면 올림, 미만이면 버림
*/
select ceil(32.2) from dual;
select floor(32.2) from dual;

select round(0.123), round (0.543) from dual;
-- 첫번쨰 항목 : 소수 이하 6자리까지 표현하므로 7을 올림처리

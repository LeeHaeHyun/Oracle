...
*/
/*
시나리오] hr계정의 사원테이블에서 담당업무가 ST_CLERK인
사원 정보를 조회할 수 있는 View를 생성하시오.
...
--3. 뷰 실행하기 : select문을 실행한 것과 동일한 결과가 인출된다.
select * from view_employees;

--4. 데이터 사전에서 뷰 확인하기
--생성시 사용한 쿼리문이 그대로 저장되는 것을 알 수 있다.
...
퀴즈] 위에서 생성한 view_employees 뷰를 아래 조건에 맞게 수정하시오
    직무 아이디 ST_MAN인 사원의 사원번호, 이름, 이메일, 매니저 아이디를
    조회할 수 있도록 수정하시오.
...
퀴즈] 사원번호, 이름, 연봉을 계산하여 출력하는 뷰를 생성하시오.
 ...
 연봉 계산식 -> (급여 + (급여 * 보너스율))*12
 뷰이름 : v_emp_salary
 단, 연봉은 세자리마다 콤마가 삽입되어야 한다.
*/
select
...
/*
- 조인을 통한 view 생성
시나리오1] 사원 테이블과 부서테이블, 지역테이블을 조인하여 다음 조건에 맞는
뷰를 생성하시오.

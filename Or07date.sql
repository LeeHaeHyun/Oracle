...
/*
months_between() : 현재 날짜와 기준 날짜 사이에 개월수를 반환한다
    형식] month_between(현재 날짜, 기준 날짜[과거 날짜]
*/
-- 2020년 1월 1일부터 지금까지 지난 개월수는??
select 
    months_between(sysdate, '2020-01-01') "기본 날짜 서식",
    months_between(sysdate, 
        to_date('2020년 01월 01일', 'yyyy"년"mm"월"dd"일"')) "to_date사용",   
    ceil(months_between(sysdate, 
        to_date('2020년 01월 01일', 'yyyy"년"mm"월"dd"일"'))) "소수점 올림처리",
...
select
 ...
 from employees
    -- order by "근속개월수2" asc; --별칭을 써도 된다
    order by trunc(months_between(sysdate, hire_date)) asc;

/*
select 결과를 정렬하기 위해 order by를 사용할때
컬럼명은 위와 같이 2가지 형태로 사용할 수 있다.
방법1 : 계산식이 포함된 컬럼을 그대로 사용한다.
방법2 : 별칭을 사용한다.
*/

/*
next_day() : 현재 날짜를 기준으로 인자로 주어진 요일에 해당하는
    미래의 날짜를 반환하는 함수

...
/*
[사용자 계정 생성 및 권한설정]
해당 부분은 DBA 권한이 있는 최고 관리자(sys,system)으로
접속한 후 실행해야 한다. 
...
*/
-- 오라클 12c이후부터는 해당 명령을 먼저 실행한 후 계정 생성
-- 미실행한 상태에서 계정을 생성하면 오류가 발생한다.
alter session set "_ORACLE_SCRIPT"=true;

-- 위 명령이 없다면 계정 생성시 C##을 계정명 앞에 항상 추가해야 한다.
create user test_user1 identified by 1234;

/*
계정 생성 직후 cmd에서 sqlplus 명령으로 접속을 시도해보면
login denied 에러가 발생된다. (권한을 주지 않았기 때문에)
*/
...
기존 암호로는 접속되지 않는다. 변경한 4321로 접속할 수 있다.
*/
...
4] Role(롤, 역할)을 통한 여러가지 권한을 동시에 부여하기
    ...
    * 우리는 실습을 위해 새롭게 생성한 계정에 connect, resource
    롤을 주로 부여한다
*/
alter session set "_ORACLE_SCRIPT"=true;
...
6] 사용자 계정 삭제
    ...
    * cascade를 명시하면 사용자 계정과 관련된 모든 데이터베이스 스키마가
    데이터 사전으로부터 삭제되고 모든 스키마 객체도 물리적으로 삭제된다.
*/
...
select * from dba_users where username=upper('test_user3'); -- 결과 없음
------------------------------------------------------------------
alter session set "_ORACLE_SCRIPT"=true;
...
/*
테이블 스페이스란? 
    디스크 공간을 소비하는 테이블과 뷰, 그리고 
    그밖의 다른 데이터베이스 객체들이 저장되는 
    장소이다. 예를 들어 오라클을 최초로 설치하면
    hr 계정의 데이터를 저장하는 user라는 테이블 스페이스가
    자동으로 생성된다.
*/
-- 테이블 스페이스 조기하기
...
select 
    ...
    trim(to_char(sum(bytes), '9,999,999,000')) "최대"
 from dba_free_space

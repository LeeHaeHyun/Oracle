...
설명: 테이블, 뷰와 같은 객체 생성 및 변경하는 쿼리문을 말한다.
/*****************/

/*
system 계정으로 연결한 후 아래 명령을 실행한다.
새로운 사용자 계정을 생성한 후 접속권한과 테이블 생성 권한 등을
부여한다.
*/
-- oracle21c 이상부터는 계정 생성전 해당 명령을 실행해야 합니다.
...
-- 해당 계정에 생성된 테이블에 목록을 저장한 시스템 테이블
...
create table tb_member (
    ...
    passwd varchar2(50), --시스템에 의해 10자리 미만을 입력해도 길게 저장됨
    username varchar2(30),
    mileage number(7,2) -- (자리수, 소수점 이해)
    -- 실수 표현, 전체 7자리, 소수 이해 2자리 표현
);    
...
기존 생성된 테이블에 새로운 컬럼 추가하기
    ->tb)member 테이블에 email 컬럼을 추가하시오.
...
/*
기존 생성된 테이블의 컬럼 수정하기
    -> tb_member 테이블의 emai 컬럼에 사이즈를 200으로 확장하시오
...
기존 생성된 테이블에서 컬럼 삭제하기
 -> tb_member 테이블의 milege 컬럼을 삭제하시오
...
/*
퀴즈] 테이블 정의서로 작성한 employees 테이블을 해당
strudy 계정에 그대로 생성하시오. 단, 제약조건을 명시하지 않습니다
*/
...
/*
tb_member 테이블에 새로운 레코드를 삽입한다.
(DML부분에서 학습할 예정)
하지만 테이블 스페이스라는 권한이 없어 삽입할 수 없다.
*/
insert into tb_member values 
    (1, 'hong', '1234', '홍길동', 'hong@naver.com');

/*
오라클 11g에서는 새로운 계정을 생성한 후
connect, resource 를 (Role)만 부여하면 
테이블 생성 및 삽입까지 되지만 그 이후 버전에서는
테이블스페이스 관련 오류가 발생한다. 따라서 아래와 같이
테이블스페이스에 대한 권한도 부여해야 한다.
해당 명령은 system 계정으로 접속한 후 실행해야 합니다.
*/    
grant unlimited tablespace to study;

-- 다시 study 계정으로 접속한 후 레코드를 삽입한다.
insert into tb_member values 
    (1, 'hong', '1234', '홍길동', 'hong@naver.com');
insert into tb_member values 
...
/*
select문을 기술할 때 where 절이 없으면 
모든 레코드를 출력하라는 명령이므로 아래에서는
모든 레코드를 가져와서 복사본 케이블을 생성한다.
*/
...
/*
DDL문 : 테이블을 생성 및 조작하는 쿼리문이다.
(Data Definition Lanuage : 데이터 정의어)
>> 매우 중요

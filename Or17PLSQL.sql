...
PL/SQL(procedural language)
    : 일반 프로그래밍 언어에서 가지고 있는 요소를 모두 가지고
    있으며 DB업무를 처리하기 위한 최적화된 언어이다.
*/
/*
- 예제1] PL/SQL 맛보기
- 화면상에 내용을 출력하고 싶을 때 on으로 설정한다.
off일때는 출력되지 않는다.
*/
set serveroutput on;
...
/*
    ...
    호스트 환경으로 빠져나오지 못한다. 즉 PL/SQL 문장을 위해
    필요하다.
...
/*
시나리오] 사원 테이블에서 사원번호가 120인 사원의 이름과
...
declare
    /*
    선언부에서 변수를 선언할 때는 테이블 생성시와 동일하게
    선언합니다.
    => 변수명 자료형 (크기)
    단, 기존에 생성된 컬럼의 타입과 크기를 참고하여 
    선언해주는 것이 좋다.
    아래 '이름'의 경우 두개의 컬럼이 합쳐진 상태이므로
    조금 더 넉넉한 크기로 설정해주는 것이 좋다.
    */
    empName varchar2(50);
    empPhone varchar2(30);
begin
    /*
    실행부 : select절에서 가져온 결과를 선언부에서
    선언한 변수에 1:1로 대입하여 값을 저장한다.
    */
    select
...
*/
-- 시나리오의 조건에 맞는 select문을 작성하시오.
select employee_id, salary, department_id from employees
    where department_id=10;

declare
    -- 사원테이블의 특정 컬럼의 타입과 크기를 그대로 참조하는 변수로 선언한다.
    empNo employees.employee_id%type; --Number(6,0) --employee_id의 타입을 쓰겠다
...
전체 컬럼을 저장후 화면에 다음 정보를 출력하시오.
단, emp_row는 사원테이블이 전체 컬럼의 저장할 수 있는 참조변수로 선언해야한다.
출력정보 : 사원번호, 이름, 이메일, 급여
*/
declare
    /*
    사원테이블 전체 컬럼을 참조하는 참조변수로 선언한다.
    이 떄 테이블명 뒤에 %rowtype을 붙여 선언한다.(전체를 참조 *)
    */
...
*/
    ...
    /*
    emp_row에는 전체 컬럼의 정보가 저장되므로 출력시 
    변수명.컬럼명 형태로 기술해야 한다.
    */
...
declare
    ...
    /*앞에서 선언한 복합변수 자료형을 통해 선언한
    복합변수로 3개의 값을 저장할 수 있다;*/
    record3 emp_3type;
begin
...
2.1의 내용을 출력한다.
3.위 내용을 완료한후 치환연산자를 사용하여 사원번호를 사용자로부터 입력받은 후 해당 사원의 정보를 출력할수있도록 수정하시오.[보류]
*/
select
...
declare
    ...
    /*앞에서 선언한 복합변수 자료형을 통해 선언한
    복합변수로 4개의 값을 저장할 수 있다;*/
    rec2 emp_4type;
begin
    -- select 절에서 가져온 4개의 값을 복합변수에 저장한다.
    -- rec2(메소드)를 부른다.
...
/*
치환연산자 : PL/SQL에서 사용자로부터 데이터를 입력받을때
사용하는 연산자로 변수 앞에 &를 붙여주면 된다. 실행시 입력창이 뜬다
*/
...
declare
    ...
    복합변수로 4개의 값을 저장할 수 있다;*/
    rec2 emp_4type;
    -- 치환 연산자를 통해 입력받은 값을 할당하여 변수를 선언한다.
    inputNum number(3);
begin
    -- 입력받은 사원번호를 쿼리에 적용한다.
    select employee_id, first_name||' '||last_name, salary, 
...
/
-- 호스트환경에서는 출력시에는 print를 사용한다.
-- 만약 출력이 안된다면 cmd에서 확인해보면 정상적으로 출력된다.
print return_var;

-- 예제7] if문 기본
/*
시나리오] 변수에 저장된 숫자가 홀수 or 짝수인지 판단하는
PL/SQL을 작성하시오.
*/
...
-- 퀴즈] 위 예제를 치한연산자를 통해 숫자를 입력받을 수 있도록 수정하시오
declare
    -- 선언부에서 숫자타입의 변수를 선언
    num number;
begin
...
begin
    ...
    elseif로 기술해야 한다. 또한 중괄호 대신 then과 end if;가
    사용된다.
    */
...
begin
    ...
    /*
    case-when문이 if문과 다른 점은 할당한 변수를
    먼저 선언한 후 문장내에서 조건을 판단하여 하나의 값을
    할당하는 방식이다. 따라서 변수를 중복으로 기술하지 않아도 된다.
    */
    dept_name :=
        case emp_dept
...
/*
반복문1 : Basic loop
    Java의 do-while문과 같이 조건 체크없이 loop진입한 후
    탈출조건이 될 때까지 반복한다. 탈출시에는 exit를 사용한다.
*/
-- 예제10] 제어문(반복문 : base loop)
declare
    num number := 0;
begin
    -- 조건 체크없이 루프로 진입한다.
    loop
        ...
        --변수를 증가시켜야 한다.
        num := num +1;
        -- num1 10을 초과하면 loop문을 탈출한다.
        -- esitms java의 break;와 동일한 역할을 한다.
        exit when (num>10);
    end loop;
end;
/
...
declare
    ...
    -- 누적합을 저장할 변수
    sumNum number := 0;
    -- 변수명으로 sum을 사용할 수 없다. 예약어(그룹함수)이므로 
    -- 오류가 발생한다.
begin
    loop
        -- 증가하는 변수 i를 누적해서 더한다.
...
/
-- 예제14] 제어문(반복문 : while) 
-- 시나리오] while loop문으로 1부터 10까지의 정수의 합을 
-- 구하는 프로그램을 작성하시오. 
declare
    i number := 1;
    sumNum number := 0;
...
/    
-- 필요없는 경우라면 declear절을 생략할 수 있다.

-- 예제16] 제어문(반복문 : for) 
-- 연습문제] for loop문으로 구구단을 출력하는 프로그램을 작성하시오
begin -- 줄바꿈 있는 
    for dan in 2...9 loop
        dbms_output.put.line(dan||'단');
        for su in 1...9 loop
...
/*
커서(Cursor)
    : select 문장에 의해 여러행이 반환되는 경우
    각 행에 접근하기 위한 객체
    선언 방법]
        Select 쿼리문, 단 into절이 없는 형태로 기술됨
    Open Cursor
        : 쿼리를 수행하라는 의미, 즉 Open할때 
        Cursor 선언시 select 문장이 실행되어
        결과셋을 얻게 된다. Cursor는 그 결과셋의 첫번째 행에
        위치하게 된다.
    Fetch~into~
        : 결과셋에 하나의 행을 읽어들이는 작업으로 결과셋을
        인출(Fetch) 후에 Cursor는 다른 행으로 이동한다.
    Close Cursor
        : 커서 닫기로 결과셋의 자원들 반납한다.
        select 문장이 모두 처리된 후 Cursor를 닫아준다.
        
    Cursor의 속성
        %Found : 가장 최근에 인출(Fetch)이 행을 Return하면
        true, 아니면 false를 반환
        &Notfound : %Found의 반대의 값을 반환한다.
        %RowCount : 지금까지 Return된 행의 개수를 반환한다.
    */
-- 시나리오] 부서테이블의 레코드를 Cursor를 통해 출력하는 PL/SQL문을 작성하시오.
declare
        --부서테이블의 전체 칼럼으 참조하는 참조변수 선언
        v_dept departments%rowtype;
        /*
        커서 선언: 부서테이블의 모든 레코드를 조회하는 select문으로
        into절이 없는 형태로 기술한다. 쿼리의 실행결과
        cur1에 저장된다.
        */
        cursor cur1 is
            select
...
begin
    /*
    해당 쿼리문을 수행해서 결과셋(ResultSet)을 가지고 온다.
...
/*
시나리오] Cursor를 사용하여 사원테이블에서 커미션이 null이 아닌 사원의 사원번호, 이름, 급여를 출력하시오. 
...
/*
컬렉션(배열)
    : 일반 프로그밍 언어에서 사용하는 배열타입을 PL/SQL에서는 테이블타입
    이라고 한다. 1,2차원 배열을 생각해보면 테이블(표)와 같은 형태이기 때문이다.
종류
    - 연관배열
    - Varray
    - 중첩테이블
1. 연관배열(Associativ Array) 
    : key와 Value의 한쌍으로 구성된 컬렉션으로 java와 해시맵과 같은 개념이다.
    key : 자료형은 주로 숫자를 사용한다. Binary_integer, pls_integer을 주로 
    사용하는데 number 타입보다 크기는 작지만, 산술연산에 빠른 특징을 
    가진다.
...
/*
시나리오] 100번 부서에 근무하는 사원의 이름을 저장하는 
연관배열을 생성하시오.
key는 숫자, value는 full_name 으로 지정하시오.
*/
-- 100번 부서에 근무하는 직원출력 : 6명
...
begin
    /*
    커서를 오픈하여 쿼리문을 실행한 후 얻어온 결과셋의
    갯수만큼 반복하여 사원명을 인출한다.
    */
    open emp_cur;
    loop
        fetch emp_cur into fname;
        -- 더이상 인출할 내용이 없으면 loop를 탈출한다.
...
/*
 ...
 고정길이를 가진 배열로써 일반 프로그래밍 언어에서 
 사용하는 배열과 동일하다. 크기에 제한이 있어서 선언할때
 크기(원소의 갯수)를 지정하면 이보다 큰 배열로 만들수없다.
    형식] Type 배열타입명 Is
            Array(배열크기) of 값의 타입;
*/
declare
...
declare
    -- VArray자료형 선언. 배열에 저장할 값은 사원아이디 컬럼을 참조하여
    -- 생성한다.
    type vaType1 is
...
형태로 사용한다. select절의 employee_id가 변수 i에 할당되고 이를 통해
인출할 수 있다.
*/
...
declare
    -- 중첩 테이블의 자료형을 생성한 후 변수 선언
    type ntType is
        table of varchar2(30);
    nt_array ntType;
begin
    -- 생성자를 통해 값을 할당한다.이 때 크기 4인 중첩 테이블이 생성된다.
    ...
    -- 크기를 확장했을때 생성자를 통해 배열의 크기를 동적으로 확장한다.
    -- 크기가 7인 중첩 테이블로 확장된다.
...
begin
   ...
   */ 
    ...
    -- 중첩 테이블의 첫번째 인덱스부터 마지막 인덱스까지 출력한다.

...
/*
1] inner join(내부조인) 
    - 가장 많이 사용되는 조인문으로 테이블 간의 
    연결조건을 모두 만족하는 레코드를 검색할때 사용한다.
    - 일반적으로 기본키(primary key)와 외래키(foreign key)를
    사용하여 join하는 경우가 대부분이다.
    - 두 개의 테이블에 동일한 이름의 컬럼이 존재하면 "테이블형.컬럼명"
    형태로 기술해야 합니다.
...
/*
시나리오] 사원테이블과 부서테이블을 조인하여
각 직원이 어떤 부서에서 근무하는지 출력하시오
단, 표준방식으로 작성하시오.
...
on
    ...
    -- 열의 정의가 애매한 경우 컬럼 앞에 테이블명을 추가합니다.
...
/*
    실행 결과에서는 소속된 부서가 없는 1명을 제외한
    나머지 106명의 레코드가 인출된다. 
    즉, inner join은 조인한 테이블에서 양쪽 모두
    만족되는 레코드만 가져오게 된다.
*/

-- 3개 이상의 테이블 조인하기
/*
시나리오] seattle(시애틀)에 위치한 부서에서 근무하는
...
/*
2] outer join (외부조인)
    outer join은 inner join과는 달리 두 테이블에 
    조인조건이 정확히 일치하지 않아도 기준이 되는
    테이블에서 레코드를 인출하는 join방식이다
    outer join을 사용할때는 반드시 outer 전에 기준이 되는
    테이블을 결정하고 쿼리문을 작성해야 한다.
...
*/
-- 실행결과를 보면 내부조인과는 다르게 107개가 인출된다. 
-- 부서가 지정되지 않은 사원까지 인출되기 떄문인데 이 경우
-- 부서쪽에 레코드가 없으므로 null이 된다.
select * from employees; -- 기준이라서 107개 
...
외부조인(left)를 통해 출력하시오. 단 오라클 방식으로 작성하시오
*/
...
/*
연습문제] 2007년에 입사한 사원을 조회하시오. 단 부서에 
배치되지 않은 직원의 경우 <부서없음>으로 출력하시오.
단, 표준방식으로 작성하시오
출력항목: 사번, 이름, 성, 부서명
*/
-- 우선 저장된 레코드 확인
...
-- 외부조인(표준방식)으로 결과 확인
-- nvl(컬럼명, '변경할 값') : null 값의 데이터를 특정한 값으로 출력
select 
...
/*
시나리오] 2007년에 입사한 사원을 조회하시오. 단, 부서에
배치되지 않은 직원의 경우 <부서없음>으로 출력하시오.
단, 오라클 방식으로 작성하시오
출력항목 : 사번, 이름, 성, 부서명
*/
...
/*
3] self join(셀프조인)
    셀프 조인은 하나의 테이블에 있는 컬럼끼리 연결해야
    하는 경우 사용한다. 즉 자기 자신이 테이블과 조인을
    맺는 것이다. 셀프 조인에서는 별칭이 테이블을 구분하는 구분자의 역할을
    하므로 굉장히 중요하다.
...
*/    
/*
여기서는 사원입장의 테이블 empClerk과 매니져입장의 테이블 empManager를
별칭으로 생성한 후 where절에 셀프조인 조건을 기술한다.
매니저도 사원이기 때문에 사원입장의 매니저 ID는 매니저 입장에서는 
사원 ID가 된다.
*/    
select
    empClerk.employee_id "사원번호", 
    empClerk.first_name||' '||empClerk.last_name "사원이름",
    empManager.employee_id "매니저 사원번호",
...
시나리오] seattle(시애틀)에 위치한 부서에서 근무하는 직원의 정보를
    ...
    출력 결과: 사원이름, 이메일, 부서아이디, 부서명, 담당 업무 아이디,
        담당 업무명, 근무지역
*/
select
...
/*
    using절에 사용된 참조 컬럼의 경우 select절에서
    테이블의 별칭을 붙이면 오히려 오류가 발생한다.
    using절에 사용된 컬럼은 양쪽의 테이블에 동시에 존재하는 컬럼이라는
    것을 전재로 작성되기 때문에 굳이 별칭을 사용할 이유가 없기 떄문이다.

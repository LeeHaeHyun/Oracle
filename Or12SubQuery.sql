...
*/
/*
시나리오] 사원테이블에서 전체 사원의 평균급여보다 낮은 급여를 받는
    사원들을 추출하여 출력하시오.
    출력항목 : 사원번호, 이름, 이메일, 연락처, 급여
*/
-- 1. 평균 급여 구하기 : 6462
select avg(salary) from employees;
-- 2. 해당 쿼리문은 문맥상 맞는 듯 하지만 그룹함수를 단일행에 적용한 잘못된 쿼리문
-- 오류가 발생한다
...
/*
복수형 서브쿼리 : 다중행 서브쿼리라고도 하고 여러개의 행을
반환하는 것으로 in, any, all, exists를 사용해야 한다.
...
/*
시나리오] 담당 업무별로 가장 높은 급여를 받는 사원의 명단을 조회하시오.
    출력 목록: 사원 아이디, 이름, 담당업무아이디, 급여
*/
-- 1단계 : 담당 업무별 가장 높은 급여를 확인한다
...
-- 복수행 연산자 any
/*
복수행 연산자 : any(어떤것이라도) 개념은 or와 비슷하다.
    메인쿼리의 비교조건이 서브 쿼리의 검색 결과와 하나 이상
    일치하면 참이 되는 연산자. 즉 둘중 하나만 만족하면 해당 레코드를 
    인출한다.
*/
/*
시나리오] 전체 사원중에서 부서번호가 20인 사원들의 급여보다
높은 급여를 받는 직원들을 인출하는 서브쿼리문 작성하시오 단 둘 중
하나만 만족하더라도 인출하시오
*/
-- 1단계 : 20번 부서 부터 급여를 확인한다
...
-- 3단계 : 둘중 하나만 만족하면 되므로 복수행 연산자 any를
-- 이용해서 서브쿼리를 만들면 된다. 즉 6000보다 크고 
-- 또는 13000보다 큰 조건으로 생성된다
select first_name, salary from employees
    where salary>any(
        select salary from 
...
/*
복수행연산자3 : all은 and의 개념과 유사하다
    메인쿼리는 비교조건이 서브쿼리의 검색 결과와 모두 일치해야
    레코드를 인출한다
*/

/*
시나리오] 전체 사원중에서 부서번호가 20인 사원들의 급여보다
높은 급여를 받는 직원들을 인출하는 서브쿼리문을 작성하시오.
...
/*
    6000이상이고 동시에 13000보다 커야 하므로 결과적으로 13000 이상인
    레코드만 인출하게 된다. 결과 : 5명
*/

/*
rownum : (가상의 번호) 테이블에서 레코드를 조회한 순서대로 순번이 부여되는
가상의 컬럼을 말합니다. 해당 컬럼은 모든 테이블에 논리적으로 존재합니다.
*/
...
/*
rownum을 우리가 정렬한 순서대로 재부여하기 위해 서브쿼리를 
사용한다.
from절에는 테이블이 들어와야 하는데, 아래의 서브쿼리에서는
사원테이블의 전체 레코드를 대상으로 하되 이름의 오름차순으로
정렬된 상태로 레코드를 가져와서 테이블처럼 사용한다.
*/
